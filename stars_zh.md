...
> `#Next.js` `#个人作品集` `#性能优化` `#Lighthouse` `#1chooo` `#1chooo-com` `#blog` `#chun-ho` `#chun-ho-lin` `#chunho` `#chunholin` `#hugo` `#hugolin` `#personal-website` `#portfolio` 
> 该项目是一个基于 Next.js、Tailwind CSS 和 TypeScript 的个人作品集网站，采用严格模式开发，集成 Turborepo 进行高效管理。支持 Markdown/MDX 内容发布，提供 RSS 订阅、站点地图、评论系统和阅读时长统计，结合 Motion 实现流畅的滚动动画与视图过渡，性能优化至 Lighthouse 近乎满分水平，兼顾 SEO 与可访问性。
